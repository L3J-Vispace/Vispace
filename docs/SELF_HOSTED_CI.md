...
2026-09-05에 `L3J-Vispace/Vispace` 전용 러너를 맥북에 설치했다. 아래 설치값은 당시 기록이다. 현재 `main`의 iOS 검증은 **수동 실행**이며 기본 실행기는 GitHub-hosted다. 소유자 MacBook을 사용하려면 실행할 브랜치와 `runner: self-hosted`를 명시해 실행한다. Core CI만 PR과 `main` push에서 자동 실행된다.
...
현재 [iOS CI 워크플로](../.github/workflows/ios-ci.yml)는 `workflow_dispatch`만 처리하고 저장소 이름을 확인한다. `runner: github-hosted`는 `macos-26`, `runner: self-hosted`는 `vispace-ios` 라벨의 MacBook을 사용한다. 선택한 MacBook이 오프라인이면 작업은 대기하며 다른 실행기로 자동 전환하지 않는다. PR을 만들거나 `main`에 push했다는 사실만으로 iOS 검증이 시작되거나 통과한 것은 아니다.

검증 범위는 모델·개인정보 선언 검사, 고정 버전과 해시를 확인한 XcodeGen 프로젝트 생성, Core 패키지 테스트, Debug 빌드와 Simulator 전체 테스트, 장소 이력 회전 회귀 최대 5회 반복, Release 빌드와 리소스 검사, 서명 없는 iOS archive, 정적 분석이다. 실기기 AR 동작과 서명·배포 검증은 별도로 수행한다. 전체 작업 한도는 60분이다.
...
실패한 self-hosted 작업은 존재하는 `.xcresult`를 맥북의 다음 경로에 ZIP으로 보존한다. 현재 워크플로는 취소되지 않은 실행의 결과를 GitHub artifact에도 업로드하며 보존 기간은 7일이다.
...
이 러너는 개인 맥북의 `dlfkd` 권한으로 저장소 코드를 실행한다. 수동 실행 권한 보유자는 선택한 브랜치의 워크플로와 실행 코드를 통해 같은 권한을 사용할 수 있다. 외부에서 받은 미검토 코드를 같은 저장소 브랜치로 옮겨 self-hosted 실행하지 않는다. 저장소 공개 전환이나 실행 조건 변경 시에도 이 권한 경계를 다시 검토한다. 수동 실행과 저장소 이름 확인은 운영 경계이며, 관리자 권한을 격리하는 샌드박스는 아니다.
