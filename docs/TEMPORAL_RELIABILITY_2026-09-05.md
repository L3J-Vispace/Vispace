...
## B08: 관측 공백 뒤 기록 연결과 자동 재식별의 범위

초기 Re-ID 경로는 같은 map/frame에서 0.75m 이내 위치·크기와 높은 공간 문맥 점수를 사용했다. 이전 위치에서 멀리 떨어진 같은 종류 물체가 동일 개체인지 새 개체인지 이 증거만으로는 결정할 수 없다. 거리 문턱을 넓히거나 이름·색상만으로 ID를 재사용하면 서로 다른 의자 등이 합쳐질 수 있으므로 그렇게 수정하지 않았다. 기존의 먼 물체/removed 물체를 근거 없이 자동 병합하지 않는 회귀 검사를 유지한다.

후속 [G01·G02 구현](FEATURE_GAPS_2026-09-05.md)은 연속 관측 증거와 명시적 사용자 확인 경로를 추가했다. 관측 공백 뒤에는 카메라의 기록 연결 화면에서 현재 물체를 이전 개체와 연결하거나 별개의 새 물체로 확인할 수 있다. 검토한 프레임·위치·기존 revision을 다음 안정된 관측에서 다시 검사하고, 유효한 확인 증거를 기존 ID와 시간 이력 처리로 전달한다. 확인 입력 접수와 영속 저장 완료는 별도 상태다. 이는 자동 외형 장거리 재식별이나 실제 이동·연결·재실행 검수의 완료를 뜻하지 않는다.
