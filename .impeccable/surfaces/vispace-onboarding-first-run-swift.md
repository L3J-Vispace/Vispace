...
Audience: an iPhone owner opening Vispace for the first time, potentially unfamiliar with spatial applications. Job: understand what the current build does, how to move the phone, what is not stored, and why camera access is about to be requested. Primary action: start the camera.

Proof/content: explain scanning, supported on-device object recognition and memory, local search and relations, geometric furniture checks, verified navigation, and data deletion. The user should hold the phone vertically and move slowly to cover the surroundings. Raw camera frames are ephemeral and are not persisted. Starting capture invokes the system-owned camera permission flow.
